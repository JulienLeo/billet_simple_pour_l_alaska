...
class CommentManager extends Manager {
    ...
    public function getComments($chapterId) { // récupération des commentaires d'un chapitre précis
        ...
        $comments = $db->prepare('SELECT id, chapter_id, author_comment, comment, DATE_FORMAT(date_comment, "%d-%m-%y à %H:%i") AS date_comment_fr FROM comments WHERE chapter_id = ? ORDER BY date_comment DESC');
    ...
    public function reportComment($commentId) { // signaler un commentaire précis
        $db = $this->dbConnect();
