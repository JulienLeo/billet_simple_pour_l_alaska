{% extends 'template.php' %}

{% block head %}
    <title>Jean Forteroche</title>
{% endblock %}

{% block content %}
    <section class="row">
        <article class="authorBio borderedText">
            <img src="img/author_page/author_portrait.png" title="Jean Forteroche" alt="portrait de Jean Forteroche">
            <h3 class="textContent"><strong>Jean Forteroche</strong></h3>
            <p class="textContent">
                Lorem ipsum dolor sit amet consectetur, adipisicing elit. Dicta officiis cupiditate nulla nostrum asperiores corporis ex a illum quidem architecto labore error, rerum deleniti repellendus maiores voluptatum sint! Aliquid, sed. Lorem ipsum dolor sit amet consectetur adipisicing elit. Nihil maiores quisquam omnis, repudiandae rerum natus at possimus veritatis quae totam harum voluptatem porro error  impedit exercitationem velit inventore tempore? Quis.
            </p>
        </article>

        <blockquote class="authorAmazon">
            <a href="https://www.amazon.com/s?k=jean+forteroche&ref=nb_sb_noss" target="_blank">Jean Forteroche sur Amazon <i class="fab fa-amazon"></i></a>
        </blockquote>
        <cite>Amazon</cite>
    </section>
{% endblock %}